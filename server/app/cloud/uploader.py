...
def make_sink() -> UploadSink | None:
    ...
    """
    configured = (
        os.environ.get("HANOI_FIREBASE_CREDENTIALS")
        or os.environ.get("GOOGLE_APPLICATION_CREDENTIALS")  # 標準ADCも受け付ける
        or os.environ.get("FIRESTORE_EMULATOR_HOST")
    )
    if not configured:
        logger.info("クラウドアップロード無効(HANOI_FIREBASE_CREDENTIALS 未設定)")
        return None
    try:
