...
- 盤面が初期盤面の**鏡像**(`mirror_board(board)` と同じサイズ列)になり、かつ
  **少なくとも1個の箱が初期状態とは別の塔にある**こと。枚数だけでなく、どのサイズが
  どの塔にあるかまで一致させる。
- 条件2は**箱の個体**で見る。左右対称な盤面(鏡像が自分自身)は条件1が最初から成立して
  しまうため、条件2が「実際に動かすこと」を要求する役割を持つ。同サイズの箱を塔間で
  入れ替えただけの盤面は盤面文字列としては初期と同一だが、箱は動いているのでクリアが
  成立する(例: `LMS/LM/LMS` は3手、`L/MS/L` は15手)。
...
- 参考検算値: クリア可能 220/512、鏡像同一視で124クラス、最短手数の最大は15手(`L/MS/L`)、
  総得点プール1282点。S1のテストでこの値と一致することを確認する。
