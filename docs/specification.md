...
- タイトル画面とランキング画面を交互に表示する。
  ...
  - ランキング画面: **1位を最上段**にした表を画面下からせり上げる演出で表示し、**最後に1位を見せて3秒**後にタイトル画面へ戻る。1画面に収まらない件数のときは最下位まで流し切ってから先頭(1位)へ戻して静止する。速度は**1行あたり1秒**を保ち(縮小や早送りはしない)、流し切る時間は最短2秒・最長120秒でクランプする。
- 操作:
  - ランキング表示中に決定 → タイトル画面へ。
  - タイトル表示中に決定 → モード選択画面へ。
  - 矢印キーは無効。
...
- 時間切れで、最新の確定かつ合法な盤面を最後に1回判定してからリザルト画面へ自動遷移する。
  時間切れ直前に押された判定は、押下時刻がタイムアップ前なら有効。
