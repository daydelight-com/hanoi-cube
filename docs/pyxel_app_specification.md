...
- ロゴ/タイトル文字、点滅する「クリックで スタート」(プロンプト表示のみ。開始は「スタート」ボタンのクリック。P5 要判断 #5)、
  ボタン「スタート」「ルール」、右上に言語切替「JA / EN」(既存仕様 §5.3 と同じ位置)、下部に自己ベスト(0 なら非表示)
- Pyxel Web は最初のクリックまで音声が出せない(`pyxel.js` が click 待ちをする)ため、タイトルの最初のクリックがそのまま音声解放を兼ねる

### 3.3 ルール説明

- **5 ページ構成**(どんなゲーム? / つみかた / うごかしかた / クリアとは? / ポイント)。文言の正である既存
  `frontend/src/i18n/strings.ts` の `RULE_PAGES`(5 ページ)を日英とも流用する(P5 要判断 #1。当初の「4 ページ」は文言流用を優先して変更)
- 各ページに**図版**を本文の上に描く(図が主役・文言 3 行が補助)。既存 frontend の `RULE_FIGURES`
  (`display/screens/ruleFigures.ts`)を 320×240 向けに移植した `screens/rule_figures.py`(データ・レイアウト、Pyxel 非依存)+
  `screens/rules.py`(描画)。箱は盤面テクスチャの実物色(大=緑/中=橙/小=白)、○✕・移動矢印・つなぎ記号(→/⇄)・
  A/B/C ラベル・バッジ(得点式)で「文字が読めなくても分かる」ことを狙う。長いキャプションが入らないため、
  コマ下キャプションは要所(あらまし・クリア)のみとし、○✕ の理由は本文 3 行に委ねる
- 「<」「>」ボタンでページ送り(先頭で「<」は無効)、ページインジケーター(●○○○○)、「もどる」。最終ページの「>」は「とじる」になる。Esc でもタイトルへ

### 3.4 ゲーム

レイアウト(320×240)。**3D ビューは全画面(320×240)に描き、HUD とボタンを重ねる**(P4 要判断 #1。
ビューポートを 320×184 に縮めるとマットが 0.77 倍になり S 箱が掴みにくくなるため):

```
┌────────────────────────────────────────┐
│ スコア 0123      のこり 0:47   しっぱい 1 │  ← 上段 HUD(高さ 16。3D ビューに重ねる)
│                                        │
│        [3D ビュー 320×240(全画面)]      │  ← A/B/C 塔(奥)、待機エリア(手前)
│                                        │
│ [タイトル]                    [はんてい] │  ← 下段ボタン(箱と重ならない隅に置く)
└────────────────────────────────────────┘
```
...
- 箱: サイズ L/M/S = 一辺 75/50/30 mm 相当の立方体。各サイズ 3 個、計 9 個。
  見た目はブース版アートワークのドット化テクスチャ(`assets/textures/`。L/M はロゴ入りを全面、S はロゴなし)。
  画像が読めない環境では色分け(L=青系, M=緑系, S=黄系)に縮退する
- マット: プレイマットのアートワーク(ドット化)を上面に貼る。画像が無ければ単色 + 塔マーカーの塗りつぶしに縮退
...
- `Box.on_draw()` は `self.box(Mat4.IDENTITY, Vec3(size,size,size), image_or_color)`。
  `Node.box` の第 3 引数は色番号のほか `pyxel.Image` を受け取れる(6 面すべてに画像全面が貼られる)ため、
  テクスチャ化に `Mesh` 差し替えは不要だった(P6a)。テクスチャの読み込みは `scene/textures.py`
...
| 10 | 箱の見た目 | **ドット化テクスチャ(P6a で変更。無ければ単色に縮退)** | 当初は「単色の立方体」だったが、ユーザー要望によりブース版アートワークを 64×64・16 色にドット化して貼る方式へ変更。タグ(AprilTag)は潰れるため箱面には貼らない。詳細は `docs/handoff/P6a.md` |
