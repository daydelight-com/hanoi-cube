...
- ロゴ/タイトル文字、点滅する「CLICK TO START」、ボタン「START」「RULES」
- Pyxel Web は最初のクリックまで音声が出せない(`pyxel.js` が click 待ちをする)ため、タイトルの最初のクリックがそのまま音声解放を兼ねる

### 3.3 ルール説明

- 既存仕様 §5.4 の4ページ構成(概要 / 並べ方のルール / クリア可能とは / 得点とコツ)を英語で
...
hanoi-cube の GitHub Release `pyxel-cube-runtime-<日付>` に添付する(要判断 #8。初回は 8.2 担当者が作る)。
...
| 2 | 日本語 UI | **初期は英語のみ** | Pyxel 標準フォントは ASCII のみ。日本語は BDF フォント(`pyxel.Font`)の同梱とライセンス確認が要る |
...
| 8 | ネイティブ wheel の配布方法 | **GitHub Release に添付** | リポジトリ肥大を避ける(Mac/Win で計 10MB 超)。CI で両 OS をビルドする案は 8.2.4 参照 |
...
| 段階 | 成果 | DoD |
|---|---|---|
| P1 | ランタイム固定・`make pyxel-site`・CI デプロイ・サンプル(c01)表示 | Pages で cube のサンプルが動く。`make check` に pyxel_app が入る |
| P2 | 3D 盤面 + ドラッグ&ドロップ(判定なし) | 9 箱を塔・待機エリア間で自由に移動でき、配置ルール違反が拒否される。§9 の非 Pyxel 層テストが通る |
| P3 | 判定・得点・タイマー・ゲーム画面 HUD | 60 秒プレイが成立し、`session` の照合テストが通る |
| P4 | タイトル / ルール / リザルト、効果音、自己ベスト | 全画面が繋がり、iPhone Safari でタッチ操作できる |
